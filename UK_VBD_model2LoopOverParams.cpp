...
int main(int argc, char **argv){
	...
	for(int SuperRep = 1;SuperRep <=23;SuperRep++){
		
		if(SuperRep <= 10){PZRadius = double(SuperRep*1000.0); SZRadius = 2*PZRadius;}
		else{PZRadius = double(10000.0 + (SuperRep-10)*5000.0); SZRadius = 2*PZRadius;}
		...
		for(int reps = 0; reps<NumOfReps ; reps++){
			...
			cout<<"This is simulation number "<<reps + 1<<", PZ = "<<PZRadius<<endl;
			...
			while(SimulationDay < NumDays){
				...
				#ifdef DAILYREADOUT
				cout<<"On day "<<SimulationDay<<endl;
